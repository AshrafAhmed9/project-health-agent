...
from src.config import RAGStatus
...
class TestS2PIntegration:
    ...
    def test_project_name_resolved(self):
        ...
        assert (
            "Titan" in self.project_data.summary.project_name
            or "S2P" in self.project_data.summary.project_name
        )
    ...
    def test_completion_percentage(self):
        assert (
            0.60 <= self.project_data.summary.percent_complete <= 0.80
        )  # Should be ~71%
    ...
    def test_resource_coverage_low(self):
        ...
        resource_signal = next(
            s for s in self.rag_result.signals if s.signal_name == "Resource Coverage"
        )
    ...
    def test_schedule_variance_detected(self):
        ...
        sched_signal = next(
            s
            for s in self.rag_result.signals
            if s.signal_name == "Schedule Performance"
        )
...
class TestPlanBIntegration:
    ...
    def test_project_name_resolved(self):
        assert (
            "UniSan" in self.project_data.summary.project_name
            or "S2P" in self.project_data.summary.project_name
        )
    ...
    def test_completion_percentage(self):
        assert (
            0.35 <= self.project_data.summary.percent_complete <= 0.55
        )  # Should be ~44%
    ...
    def test_milestones_include_training_phase(self):
        ...
        training = [
            m for m in self.project_data.milestones if "Training Phase I" in m.task_name
        ]
