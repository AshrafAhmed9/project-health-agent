...
def generate_trend_chart(weeks: List[str], s2p_scores: List[float], planb_scores: List[float], output_path: Path):
    """
    ...
    ax.plot(weeks, s2p_scores, color='#1B2A4A', marker='o', linewidth=3, markersize=8, label='S2P Project (Titan)')
    ax.plot(weeks, planb_scores, color='#0096D6', marker='s', linewidth=3, markersize=8, label='Project Plan B (UniSan)')
    
    # Titles and formatting
    ax.set_title('Project Health Score Trends (Composite 3-Week View)', fontsize=14, fontweight='bold', pad=15)
    ...
    for i, txt in enumerate(s2p_scores):
        ax.annotate(f"{txt:.0f}", (weeks[i], s2p_scores[i] + 2), fontsize=10, fontweight='bold', ha='center', color='#1B2A4A')
    for i, txt in enumerate(planb_scores):
        ax.annotate(f"{txt:.0f}", (weeks[i], planb_scores[i] - 4), fontsize=10, fontweight='bold', ha='center', color='#0096D6')
...
def generate_resource_donut(s2p_assigned: int, s2p_unassigned: int, planb_assigned: int, planb_unassigned: int, output_path: Path):
    """
    ...
    rects1 = ax.bar([i - width/2 for i in x], assigned, width, label='Assigned Tasks', color='#2D4A7A')
    rects2 = ax.bar([i + width/2 for i in x], unassigned, width, label='Unassigned Tasks', color='#E83E3E')
    
    ax.set_title('Resource Assignment Gaps (Active Tasks)', fontsize=12, fontweight='bold', pad=15)
