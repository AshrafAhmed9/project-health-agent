...
class PPTXGenerator:
    ...
    def _add_slide_header(self, slide, title: str, subtitle: str):
        ...
        # Title text box
        txBox = slide.shapes.add_textbox(Inches(0.5), Inches(0.1), Inches(12.333), Inches(0.9))
        tf = txBox.text_frame
        tf.word_wrap = True
    ...
    def _add_bullet_list(self, slide, bullets: list, left: float, top: float, width: float, height: float):
        """
    ...
    def build_deck(self, slide_data: Dict[str, Any], output_path: Path, trend_chart_path: Path, resource_chart_path: Path):
        """
        ...
        self._add_slide_header(slide1, data1.get("title", "Portfolio Health Overview"), data1.get("subtitle", ""))
        self._add_bullet_list(slide1, data1.get("bullets", []), 0.5, 1.4, 7.0, 5.0)
        
        # Add Card for Project S2P (Titan)
        card1 = slide1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(8.0), Inches(1.5), Inches(4.8), Inches(2.3))
        card1.fill.solid()
        ...
        card2 = slide1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(8.0), Inches(4.2), Inches(4.8), Inches(2.3))
        card2.fill.solid()
        ...
        self._add_slide_header(slide2, data2.get("title", "Portfolio Trajectory Trends"), data2.get("subtitle", ""))
        self._add_bullet_list(slide2, data2.get("bullets", []), 0.5, 1.4, 6.0, 5.0)
        
        # Insert trend chart image
        if trend_chart_path.exists():
            slide2.shapes.add_picture(str(trend_chart_path), Inches(7.0), Inches(1.5), Inches(5.8), Inches(4.8))
        ...
        self._add_slide_header(slide3, data3.get("title", "Critical Risk Matrix"), data3.get("subtitle", ""))
        ...
        matrix_bg = slide3.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(8.5), Inches(1.5), Inches(4.3), Inches(4.5))
        ...
        self._add_slide_header(slide4, data4.get("title", "Schedule Performance Deep-Dive"), data4.get("subtitle", ""))
        ...
        self._add_slide_header(slide5, data5.get("title", "Resource & Dependency Analysis"), data5.get("subtitle", ""))
        self._add_bullet_list(slide5, data5.get("bullets", []), 0.5, 1.4, 6.0, 5.0)
        
        # Insert resource chart image
        if resource_chart_path.exists():
            slide5.shapes.add_picture(str(resource_chart_path), Inches(6.8), Inches(1.6), Inches(6.0), Inches(3.8))
        ...
        self._add_slide_header(slide6, data6.get("title", "Strategic Recommendations"), data6.get("subtitle", ""))
        ...
        self._add_slide_header(slide7, data7.get("title", "30-Day Outlook & Forecast"), data7.get("subtitle", ""))
