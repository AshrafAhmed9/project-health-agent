...
def analyze(
    data_dir: Path = typer.Option(
        settings.data_dir, help="Directory containing project Excel files"
    ),
    ref_date_str: str = typer.Option(
        "2026-07-02", help="Reference date for analysis (YYYY-MM-DD)"
    ),
):
    """
    ...
    files = [
        f
        for f in files
        if f.name not in ("~$S2P Project.xlsx", "~$Project Plan B.xlsx")
    ]
    ...
    table = Table(
        title=f"Project Health Analysis Dashboard (Reference Date: {ref_date})"
    )
    ...
    for f in files:
        try:
            ...
            active_count = (
                proj_data.summary.in_progress_count
                + proj_data.summary.not_started_count
                + proj_data.summary.on_hold_count
            )
...
def report(
    ...
    for f in files:
        try:
            ...
            report_content = agent.generate_weekly_report(
                proj_data, rag_res, trend=None
            )
            ...
            console.print(
                f"[green]✓ Generated report for {proj_data.summary.project_name} for date {ref_date_str}[/green]"
            )
        except Exception as e:
...
def full_run(
    ...
    """
    console.print(
        "[bold navy]🏥 Running Zycus Project Health Reporting Pipeline...[/bold navy]"
    )
    ...
    for week_str in simulated_weeks:
        ...
        for f in files:
            ...
            try:
                ...
                console.print(
                    f"[red]⚠️ Failed processing {f.name} for week {week_str}: {e}[/red]"
                )
                import traceback
    ...
    generator.build_deck(
        slide_content, pptx_path, trend_chart_path, resource_chart_path
    )
