...
def analyze_dependencies(project_data: ProjectData) -> SignalResult:
    ...
    low_float_tasks = [
        t for t in active_tasks if t.total_float is not None and t.total_float <= 3
    ]
    ...
    detail = (
        f"Dependency risk: {critical_delayed} of {total_critical} critical path tasks are delayed. "
