...
def analyze_weekly_trends(
    current: RAGResult, historical: List[RAGResult]
) -> TrendAnalysis:
    """
    ...
    for s in current.signals:
        prev_sig = next(
            (ps for ps in last_week.signals if ps.signal_name == s.signal_name), None
        )
        if prev_sig:
            if prev_sig.score >= 70 and s.score < 40:
                emerging_risks.append(
                    f"Critical decline in {s.signal_name} (fell from {prev_sig.score:.0f} to {s.score:.0f})"
                )
            elif prev_sig.score >= 70 and s.score < 70:
                emerging_risks.append(
                    f"Decline in {s.signal_name} (fell from {prev_sig.score:.0f} to {s.score:.0f})"
                )

    # Add general observations
    insights.append(
        f"Composite health score changed by {score_delta:+.1f} points since last week."
    )
    if direction == "DECLINING":
        insights.append(
            "Project trajectory is declining. Urgent corrective actions needed."
        )
    elif direction == "IMPROVING":
        insights.append(
            "Project trajectory is improving. Corrective actions are yielding results."
        )
