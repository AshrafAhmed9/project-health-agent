...
from src.config import (
    RAGStatus, RAG_GREEN_THRESHOLD, RAG_AMBER_THRESHOLD,
    RED_OVERRIDE_SINGLE_HEAVY, RED_OVERRIDE_SINGLE_MEDIUM,
    RED_OVERRIDE_MULTI_COUNT, AMBER_OVERRIDE_MULTI_COUNT,
    LOW_CONFIDENCE_THRESHOLD, INSUFFICIENT_DATA_THRESHOLD
)
from src.ingestion.normalizer import ProjectData
from src.analysis.schedule_analyzer import analyze_schedule, SignalResult, score_to_rag
...
class RAGEngine:
    ...
    def compute_rag(self, project_data: ProjectData) -> RAGResult:
        ...
        heavy_signals_low = [s for s in signals if s.weight >= 0.20 and s.score <= RED_OVERRIDE_SINGLE_HEAVY]
        # Check medium weights (Velocity, Blocker, Dependency) -> weight >= 15%
        medium_signals_low = [s for s in signals if s.weight >= 0.15 and s.score <= RED_OVERRIDE_SINGLE_MEDIUM]
        ...
        if data_confidence < INSUFFICIENT_DATA_THRESHOLD:
            ...
            confidence_warning = "INSUFFICIENT DATA: Data quality is too low for reliable analysis."
        elif data_confidence < LOW_CONFIDENCE_THRESHOLD:
            confidence_warning = "LOW CONFIDENCE: High volume of missing or unparseable fields."
