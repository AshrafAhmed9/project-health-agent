...
def analyze_schedule(project_data: ProjectData) -> SignalResult:
    ...
    for task in project_data.milestones:
        if (
            task.variance_days is not None
            and task.variance_days < max_negative_variance
        ):
    ...
    if max_negative_variance < -20:
        score = min(score, 30)
        detail_var = (
            f"Critical schedule slippage of {abs(max_negative_variance)} days detected."
        )
    elif max_negative_variance < -10:
        score = min(score, 50)
        detail_var = (
            f"Moderate schedule slippage of {abs(max_negative_variance)} days detected."
        )
    else:
